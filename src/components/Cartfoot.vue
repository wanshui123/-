<template>
  <div class="cartList" v-show="this.totalMount">
    <!-- 购物车弹出 -->
    <van-popup v-model="show" position="bottom" :style="{ height: '40%' }">
      <div class="header">
        <span>已选商品</span>
        <div>
          <van-icon name="delete-o" size="0.18rem" color="#333" /><span
            @click="clearCart"
            >清空购物车</span
          >
        </div>
...
export default {
  ...
  mounted() {
    ...
    let num = localStorage.getItem("totalCount");
    console.log(num);
    if (num > 0) {
      this.totalMount = true;
    } else {
      this.totalMount = false;
    }
  },
  updated() {
    setStorage({
